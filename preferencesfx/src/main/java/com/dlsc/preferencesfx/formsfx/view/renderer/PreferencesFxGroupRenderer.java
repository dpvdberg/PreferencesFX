...
public class PreferencesFxGroupRenderer {
  ...
  public void layoutParts() {
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      // Add description when one is set
      if (field.getLabelDescription() != null) {
        rowAmount++;
        grid.add(field.getLabelDescription(), 0, i + (rowAmount), 2, 1);
        GridPane.setValignment(field.getLabelDescription(), VPos.CENTER);
        field.getLabelDescription().getStyleClass().add(styleClass.toString() + "-label");
        GridPane.setMargin(
                field.getLabelDescription(),
                new Insets(0, 0, PreferencesFxFormRenderer.SPACING, 3 * PreferencesFxFormRenderer.SPACING)
        );
      }
      ...
      if (i == fields.size() - 1) {
        ...
        if (field.getLabelDescription() != null) {
          GridPane.setMargin(
                  field.getLabelDescription(),
                  new Insets(0, 0, PreferencesFxFormRenderer.SPACING * 5, 3 * PreferencesFxFormRenderer.SPACING)
          );
        } else {
          GridPane.setMargin(
                  c.getNode(),
